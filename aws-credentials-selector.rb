  ...
  ini.each_section do |section|
    ini[section].each do |k, v|
      ...
      when AWS_ACCESS_KEY_ID
        ary << "export AWS_ACCESS_KEY_ID=#{v}"
      when AWS_SECRET_ACCESS_KEY
        ary << "export AWS_SECRET_ACCESS_KEY=#{v}"
      end
    end
  end
  puts ary.join("\n")
...
def main() 
  ...
  ini = IniFile.load(cred_path)
  if ini.nil?
    puts "AWS Credential file not found, check your the file, '#{cred_path}'"
    exit 1
  end
  unless ini.has_section?(ARGV[0])
    puts "You have to input exist \"section\", not exist '#{ARGV[0]}'"
    puts "  '#{ini.sections.join(', ')}'"
    exit 1
  end
  ret = []
  ini[ARGV[0]].each do |k, v|
    case k
    when AWS_ACCESS_KEY_ID
      ret << "export AWS_ACCESS_KEY_ID=#{v}"
    when AWS_SECRET_ACCESS_KEY
      ret << "export AWS_SECRET_ACCESS_KEY=#{v}"
    end
  end
  puts ret.join("\n")
end
